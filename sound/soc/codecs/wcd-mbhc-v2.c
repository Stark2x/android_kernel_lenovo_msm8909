...
//Modify by chaofubang to be compatible with iphone OMTP headset SW00183704 2016-04-28 start
#define HS_DETECT_PLUG_TIME_MS (1200)
...
//Added by chaofubang to fix handfree voice call no sound after removing headset. A6500M-280 2016-06-07 start
extern int ext_spk_mode;
//Added by chaofubang to fix handfree voice call no sound after removing headset. A6500M-280 2016-06-07 end
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		//Modify by chaofubang to fix 3-pole headset detect error A6501M-103 2016-05-04 start
		...
		//Modify by chaofubang to fix 3-pole headset detect error A6501M-103 2016-05-04 end
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	WCD_MBHC_REG_READ(WCD_MBHC_HS_COMP_RESULT, hs_comp_res);
	pr_info("%s btn_result: %d hs_comp_res: %d  rc: %d\n", __func__, btn_result, hs_comp_res, rc);
	if (!rc) {
...
static void wcd_mbhc_detect_plug_type(struct wcd_mbhc *mbhc)
{
	...
	int cross_conn;
	int try = 0;
	int rc;//Modified by zhangkaibi to improve handphone detect speed A6501M-2872 20160803
	...
	if (mbhc->mbhc_cb->mbhc_micbias_control)
		...
		wcd_enable_curr_micbias(mbhc, WCD_MBHC_EN_MB);
	/*Modified by zhangkaibi to fix headset type detect error A6501M-2898 20160804 begain*/
	/* Re-initialize button press completion object */
	reinit_completion(&mbhc->btn_press_compl);
	WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_FSM_EN, 1);
	/*Modified by zhangkaibi to fix headset type detect error A6501M-2898 20160804 end*/
	/*Modified by zhangkaibi to improve handphone detect speed A6501M-2872 20160803 begain*/
	rc = wait_for_completion_timeout(&mbhc->btn_press_compl,
			msecs_to_jiffies(WCD_MBHC_BTN_PRESS_COMPL_TIMEOUT_MS));
	pr_info("%s: rc: %d",__func__,rc);
	/* The value of rc is detected time which the plugged earphone have been button */
	if(rc){
		plug_type = MBHC_PLUG_TYPE_HEADPHONE;
		wcd_mbhc_find_plug_and_report(mbhc, plug_type);
	}else{
	...
	/*Modified by zhangkaibi to improve handphone detect speed A6501M-2872 20160803 end*/
	pr_info("%s: leave\n", __func__);
}
